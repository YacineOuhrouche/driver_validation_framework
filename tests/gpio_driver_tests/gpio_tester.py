...
class GPIODriverTester:
    ...
    def configure_rising_interrupt(self, pin: str) -> str:
        return self.device.send_command(
            f"GPIO_INTERRUPT {pin} RISING"
        )

    # Configure falling edge interrupt.
    def configure_falling_interrupt(self, pin: str) -> str:
        return self.device.send_command(
            f"GPIO_INTERRUPT {pin} FALLING"
        )

    # Read GPIO interrupt status.
    def interrupt_status(self, pin: str) -> str:
        return self.device.send_command(
            f"GPIO_INTERRUPT_STATUS {pin}"
        )
